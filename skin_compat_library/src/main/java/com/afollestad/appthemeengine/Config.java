...
public final class Config extends ConfigBase {
    ...
    //TODO--- tag为primary_color的 通过new创建的view 颜色取值方法---
    public static int getPrimaryColor(){
//        final SharedPreferences prefs = prefs(mContext, mKey);
//        return prefs.getInt(KEY_PRIMARY_COLOR, 0);
        if (mContext!=null && mKey!=null){
            return prefs(mContext, mKey).getInt(KEY_PRIMARY_COLOR, 0);
        }else {
            return 0;
        }
    }
